...
- `zalo_users` — bảng của Thich24hNotify (zalo_oa_user_id, status, is_follow, phone) — BE ghi phone khi ingest UTM; khi `/consents` báo `oa_followed=true` và có `oa_user_id` → BE chỉ sync `is_follow=TRUE` (không đụng phone/status).
