    ...
    connect: async () => {
      ...
      return {
        ...
        query: async (sql, params) => {
          ...
          if (
            /SELECT event_id, applied, customer_id FROM utm_events WHERE event_id = \$1/.test(s)
          ) {
            return dup
          ...
          if (/SELECT MAX\(occurred_at\) AS ts FROM utm_events WHERE oa_user_id = \$1/.test(s)) {
            return latestTs == null ? { rows: [] } : { rows: [{ ts: new Date(latestTs) }] };
          }
...
const sqls = (calls) =>
  calls.filter((c) => c.conn && c.sql).map((c) => c.sql.replace(/\s+/g, ' ').trim());
...
test('ingest success: canonical phone upserted + utm_events applied, never sets customer_id', async () => {
  ...
  const upsertParams = calls.find(
    (c) => c.conn && c.sql?.includes('INSERT INTO zalo_users')
  ).params;
...
test('maybeIngestPhoneShared skips phone-missing', async () => {
  const { pool, calls } = makeFake();
  const res = await maybeIngestPhoneShared({
    pool,
    user_id_by_app: 'app-1',
    phone: null,
    occurredAt: 'x',
  });
...
test('maybeIngestPhoneShared writes ingest for linked user with canonical phone + UTM_SOURCE', async () => {
  ...
  assert.equal(
    res.eventId,
    deriveUtmEventId({ uid: 'oa-1', phone: '0901234567', occurredAt: '2026-09-21T09:00:00.000Z' })
  );
});
